...
<div
	...
	class="absolute w-full h-full inset-0 bg-background z-30 p-4 grid"
>
		...
		<div transition:fade={{ duration: 150 }} class="flex flex-col justify-between items-center h-full gap-10 col-start-1 col-end-2 row-start-1 row-end-2">
			<div class="flex flex-col justify-center w-full">
				<div class="flex items-center gap-2 p-4 py-2">
					<div class="relative">
						<svg class="-m-2" width="128" height="128" viewBox="0 0 192 192" fill="none" xmlns="http://www.w3.org/2000/svg" style:filter="drop-shadow(0px 4px 4px var(--color-shadow))" >
				...
				<div class="flex flex-col items-center gap-6">
					<div class="flex gap-12">
						<Metric value={'8,76'} unit={'€'} label={'Saldo'} color={'info'} />
						<Metric value={21910} label={'Bónus'} color={'info'} />
					</div>
					<div>
						<div class="flex items-center gap-1 justify-center">
							<IconTicket size={28} stroke={1.9} class="text-info -my-1" />
							<div class="text-info font-bold text-lg">Passe Anual</div>
						</div>
						<div class="text-2xs text-label font-medium -mt-[2px]">Válido até 20 de Março de 2024</div>
					</div>
				</div>
			</div>
