			...
			<div>
								...
								<g filter="url(#filter0_d_102_664)">
									...
									<g filter="url(#filter1_d_102_664)">
										<circle
											...
											cy="96"
											r="50"
											class="group-active:fill-neutral-200 fill-white transition-colors"
										/>
									</g>
								</g>
							</svg>
							<span
								class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-2xl font-semibold text-neutral-500"
				...
				<div class="flex flex-col font-semibold px-2 gap-4">
					...
					<SettingsEntry
						icon={IconTool}
						text={'Configurações'}
						subtext={'Definições da aplicação'}
						on:click={() => openPage = 'settings'}
					/>
					<SettingsEntry
						icon={IconInfoCircle}
						text={'Sobre'}
						subtext={'Informação acerca da aplicação'}
						on:click={() => openPage = 'info'}
					/>
				</div>
			</div>
