...
<div
	...
	class="absolute w-full h-full inset-0 bg-white z-30 p-4 grid"
>
	<div class="absolute top-4 right-4 flex justify-end">
		<button on:click={() => dispatch('close')}>
			<IconX class="text-[#808080]" size={24} />
		</button>
	</div>
	{#if openPage !== null}
		<div transition:fade={{ duration: 150 }} class="absolute top-4 left-4 flex justify-end">
			<button on:click={() => openPage = null}>
				<IconArrowLeft class="text-neutral-500" size={24} />
			</button>
		</div>
	{/if}
	{#if openPage === null}
		<div transition:fade={{ duration: 150 }} class="flex flex-col justify-between items-center h-full gap-10 col-start-1 col-end-2 row-start-1 row-end-2">
			<div class="flex flex-col justify-center w-full">
				<div class="flex items-center gap-2 p-4 py-2">
					<div class="relative">
						<svg class="-m-2" width="128" height="128" viewBox="0 0 192 192" fill="none" xmlns="http://www.w3.org/2000/svg" style:filter="drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))" >
							<g filter="url(#filter0_d_102_664)">
								...
								<g filter="url(#filter1_d_102_664)">
									<circle cx="96" cy="96" r="50" class="group-active:fill-neutral-200 fill-white transition-colors" />
								</g>
							</g>
						</svg>
						<span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-2xl font-semibold text-[#808080]">
							{$user?.name.charAt(0)}
						</span>
					</div>
					<div>
						...
						<div class="text-xs font-semibold text-[#B3B3B3] -mt-[2px]">{$user?.email}</div>
					</div>
				</div>
				<div class="flex flex-col items-center gap-6">
					<div class="flex gap-12">
						<Metric value={'8,76'} unit={'€'} label={'Saldo'} color={'[#808080]'} />
						<Metric value={21910} label={'Bónus'} color={'[#808080]'} />
					</div>
					<div>
						<div class="flex items-center gap-1 justify-center">
							<IconTicket size={28} stroke={1.9} class="text-[#808080] -my-1" />
							<div class="text-[#808080] font-bold text-lg">Passe Anual</div>
						</div>
						<div class="text-2xs text-[#B3B3B3] font-medium -mt-[2px]">Válido até 20 de Março de 2024</div>
					</div>
				</div>
			</div>
			...
			<button class="flex flex-col items-center mb-3">
				<IconLogout2 class="text-primary mr-2" size={32} />
				<span class="text-2xs font-medium text-[#B3B3B3] text-center leading-none max-w-[70px]">SAIR</span>
			</button>
		</div>
