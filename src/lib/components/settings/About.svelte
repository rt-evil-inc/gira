<script>
	import { version } from '$app/environment';
	import { safeInsets } from '$lib/stores';
</script>

<div class="flex flex-col h-screen p-5" style:padding-top={$safeInsets.top + 48 + 'px'}>
	<div class="text-3xl font-bold text-info pt-1">Sobre</div>
	<div class="flex flex-col h-full px-5 py-10 gap-10 items-center">
		<div class="flex flex-col w-full gap-3 font-medium text-info leading-tight">
			<span>Esta aplicação é uma re-implementação das funcionalidades da aplicação Gira.</span>
			<span>A aplicação não é oficial, não estando associada de modo algum à EMEL ou à Câmara Municipal de Lisboa.</span>
		</div>
		<div class="flex flex-col w-full gap-4">
			<span class="font-bold text-info text-lg">Desenvolvido por:</span>
			<a href="https://github.com/rodrigohpalmeirim" class="flex gap-3 items-center">
				<img src="https://avatars.githubusercontent.com/u/34187774" alt="" class="w-14 h-14 rounded-full" />
				<div class="flex flex-col">
					<span class="font-bold text-primary">Rodrigo Palmeirim</span>
					<span class="font-semibold text-label text-xs -mt-[2px]">gira@rodlabs.dev</span>
				</div>
			</a>
			<a href="https://github.com/ttmx" class="flex gap-3 items-center">
				<img src="https://avatars.githubusercontent.com/u/12669467" alt="" class="w-14 h-14 rounded-full" />
				<div class="flex flex-col">
					<span class="font-bold text-primary">Tiago Teles</span>
					<span class="font-semibold text-label text-xs -mt-[2px]">gira@tteles.dev</span>
				</div>
			</a>
		</div>
		<a href="https://github.com/ttmx/gira-app" class="flex items-center gap-3 p-4 bg-background active:bg-background-secondary transition-colors rounded-2xl" style:box-shadow="0px 0px 12px 0px var(--color-shadow)">
			<svg class="w-7 h-6" width="72" height="63" viewBox="0 0 72 63" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path d="M52.875 0C45.915 0 39.813 3.60882 36 9.08956C32.187 3.60882 26.085 0 19.125 0C9.387 0 0 8.2616 0 20.249C0 33.0734 9.204 43.7889 17.484 50.9285C22.6849 55.3737 28.3309 59.2702 34.332 62.556L34.416 62.607L34.44 62.616L34.437 62.625C34.926 62.88 35.463 63.0029 36 62.9999C36.537 63.0029 37.074 62.877 37.563 62.625L37.56 62.616L37.584 62.607L37.668 62.556C43.6691 59.2702 49.3151 55.3737 54.516 50.9285C62.796 43.7889 72 33.0734 72 20.249C72 8.2616 62.613 0 52.875 0ZM50.118 45.8228C45.7361 49.5721 41.0125 52.9025 36.009 55.7703L35.997 55.7643L35.985 55.7703C30.9833 52.8997 26.2599 49.5695 21.876 45.8228C13.845 38.9021 6.735 29.9296 6.735 20.255C6.735 11.9934 13.098 6.75567 19.11 6.75567C25.29 6.75567 30.852 11.1925 32.742 17.8071C33.171 19.2921 34.53 20.198 36 20.195C37.47 20.198 38.829 19.2891 39.255 17.8071C41.145 11.1925 46.707 6.75567 52.887 6.75567C58.899 6.75567 65.262 11.9934 65.262 20.255C65.262 29.9296 58.152 38.9021 50.118 45.8228Z" fill="#BF3989"/>
			</svg>
			<span class="font-semibold text-info text-sm">Patrocina este projeto no GitHub</span>
		</a>
		<div class="flex flex-col grow items-center justify-end">
			<a href="https://github.com/ttmx/gira-app">
				<svg class="w-12" width="120" height="118" viewBox="0 0 120 118" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path fill-rule="evenodd" clip-rule="evenodd" d="M59.8212 0C26.7416 0 0 27.0417 0 60.4959C0 87.2376 17.1343 109.874 40.9041 117.886C43.8759 118.488 44.9645 116.584 44.9645 114.982C44.9645 113.58 44.8665 108.773 44.8665 103.764C28.2257 107.37 24.7604 96.5523 24.7604 96.5523C22.0861 89.5411 18.1237 87.7391 18.1237 87.7391C12.6771 84.0332 18.5204 84.0332 18.5204 84.0332C24.562 84.4339 27.7322 90.243 27.7322 90.243C33.0796 99.4568 41.6963 96.8534 45.1629 95.2506C45.6576 91.3443 47.2433 88.6401 48.9269 87.1381C35.6547 85.7356 21.6906 80.5276 21.6906 57.2902C21.6906 50.6798 24.0661 45.2714 27.8302 41.0652C27.2363 39.5632 25.1559 33.3522 28.4253 25.0394C28.4253 25.0394 33.4763 23.4365 44.8653 31.2491C49.7413 29.9249 54.7699 29.2512 59.8212 29.2456C64.8722 29.2456 70.0212 29.9474 74.7759 31.2491C86.1661 23.4365 91.2171 25.0394 91.2171 25.0394C94.4865 33.3522 92.4049 39.5632 91.811 41.0652C95.6743 45.2714 97.9518 50.6798 97.9518 57.2902C97.9518 80.5276 83.9878 85.6348 70.6163 87.1381C72.7959 89.0408 74.6767 92.646 74.6767 98.3555C74.6767 106.468 74.5788 112.979 74.5788 114.981C74.5788 116.584 75.6686 118.488 78.6392 117.887C102.409 109.873 119.543 87.2376 119.543 60.4959C119.641 27.0417 92.8016 0 59.8212 0Z" fill="#24292F"/>
				</svg>
			</a>
			<span class="font-semibold text-info text-xs">Projeto open-source sob a licença GPL-3.0</span>
		</div>
	</div>
	<a href="https://github.com/ttmx/gira-app/releases" class="absolute bottom-5 left-5 font-semibold text-label text-xs">v{version}</a>
</div>