import type { GiraApiInterface, M, Q } from '.';

export class MockApi implements GiraApiInterface {
	async reserveBike(serialNumber: string): Promise<M<['reserveBike']>> {
		return {
			reserveBike: true,
		};
	}

	async getStationInfo(stationId: string): Promise<Q<['getBikes', 'getDocks']>> {
		// TODO
		return { getBikes: [], getDocks: [] };
	}

	async cancelBikeReserve(): Promise<M<['cancelBikeReserve']>> {
		return { cancelBikeReserve: true };
	}

	async startTrip(): Promise<M<['startTrip']>> {
		return {
			startTrip: true,
		};
	}

	async rateTrip(tripCode: string, tripRating: number, tripComment?: string, tripAttachment?: File): Promise<M<['rateTrip']>> {
		return {
			rateTrip: true,
		};
	}

	async tripPayWithNoPoints(tripCode: string): Promise<M<['tripPayWithNoPoints']>> {
	...
	async tripPayWithPoints(tripCode: string): Promise<M<['tripPayWithPoints']>> {
		// TODO check what this actually returns?
		return {
	...
	async fullOnetimeInfo(): Promise<Q<['getStations', 'client', 'activeUserSubscriptions', 'activeTrip', 'unratedTrips', 'tripHistory']>> {
		return {
			...
			tripHistory: (await this.getTripHistory(0, 10)).tripHistory,
		};
	}

	async getTripHistory(pageNum: number, pageSize: number): Promise<Q<['tripHistory']>> {
		return {
			tripHistory: [
				{
					'code': '6JWFVI9O8N',
	...
	async getActiveTripInfo(): Promise<Q<['activeTrip']>> {
		return {
			activeTrip: null,
		};
	}
