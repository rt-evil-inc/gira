...
type Q<T extends keyof Query> = {[K in T]:Query[T]};
...
export async function getStations(): Promise<Q<'getStations'>> {
	const req = post<'getStations'>({
		...
		'variables': {},
		'query': 'query getStations {getStations { dockList { ledStatus, lockStatus, serialNumber, assetType, assetStatus, assetCondition, parent, warehouse, zone, location, latitude, longitude, code, name, description, creationDate, createdBy, updateDate, updatedBy, defaultOrder, version }, code, description, latitude, longitude, name, bikes, docks, serialNumber }}',
	});
	return req;
}
...
export async function updateStations() {
	getStations().then(maybeStations => {
        ...
        name === undefined || description === null || description === undefined ||
