...
<div class="max-w-sm px-4 w-full">
	<div class="bg-white rounded-lg shadow-lg max-w-sm w-full flex flex-col items-center p-6">
		<div class="text-center text-lg text-neutral-800 font-medium mb-2 rounded-md">Bem vindo ao Gira+</div>
		<form class="flex flex-col w-full gap-4" on:submit={loginWrapper}>
			<div class="w-full">
				<div class="text-sm text-neutral-700 pb-1">Email</div>
				<input type="email" required bind:value={email} placeholder="example@example.pt" class="w-full px-2 h-12 rounded-md border border-neutral-200 bg-neutral-100 focus:ring-primary focus:border-primary">
			</div>
			<div class="w-full">
